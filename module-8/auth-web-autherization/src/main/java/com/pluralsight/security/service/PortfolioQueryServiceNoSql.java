...
public class PortfolioQueryServiceNoSql implements PortfolioQueryService {
	...
	private final PortfolioAccessControlRepository porfolioAccessControlRepo;
	...
	@Override
	@PreAuthorize("hasPermission(#id, 'com.pluralsight.security.api.entity.Portfolio', 'READ')")
	...
	@Override
	public List<SharedPortfolioDto> getSharedPortfolios() {
		List<PortfolioAccessControl> accessControl = this.porfolioAccessControlRepo.findAllByUsername(getUsername());
		List<String> sharedPortfolioIds = accessControl.stream().map(PortfolioAccessControl::getPortfolioId).collect(Collectors.toList());
		List<Portfolio> sharedPortfolios = (List<Portfolio>) portfolioRepository.findAllById(sharedPortfolioIds);
		List<SharedPortfolioDto> sharedPortfolioDtos = new ArrayList<>();
		for(Portfolio portfolio : sharedPortfolios) {
			CryptoUser user = this.userRepository.findByUsername(portfolio.getUsername());
			if(user == null) {
				continue;
			}
			sharedPortfolioDtos.add(new SharedPortfolioDto(user.getFirstName()+ " "+user.getLastName(), portfolio.getId()));
		}
		return sharedPortfolioDtos;
	}	
