...
public class PortfolioQueryController {
	...
	public ModelAndView searchTransactionsForPortfolio(@RequestParam("symbol") Optional<String> symbol) {
		ListTransactionsDto transactions = null;
		if(!symbol.isPresent()) {
			transactions = portfolioService.getPortfolioTransactions();
		} else {
			transactions = portfolioService.getPortfolioTransactions(symbol.get(), "user");
		}
		ModelAndView model = new ModelAndView();
		model.addObject("transactions",transactions.getTransactions());
		model.addObject("selected",new DeleteTransactionsDto());
